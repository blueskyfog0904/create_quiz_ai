...
- 결제 기능은 `PAYMENTS_ENABLED=true`일 때만 동작한다.
- schema, 약관, Toss 계약, 결제수단, 웹훅과 대사 작업을 모두 확인하기 전에는 기능을 켜지 않는다.
- secret, payment key, 웹훅 token은 문서·스크린샷·일반 로그에 기록하지 않는다.
- 운영 DB와 Toss live 환경에서의 결제·취소는 별도 승인된 담당자만 수행한다.
...
4. 카드와 카카오페이·네이버페이·페이코·토스페이만 허용하는 전용 MID·결제위젯 설정
...
운영 키는 모두 같은 환경과 MID 계약에 속해야 한다. test key와 live key가 섞이면 서버가 결제를 거부하도록 유지한다.
...
- 한 번에 최대 50건만 처리한다.
- `confirming`, `fulfillment_pending`, 취소·환불 재처리 상태를 provider 조회 결과와 대조한다.
- 금액·주문·MID가 다르면 자동 지급·환불하지 않고 `manual_review`로 보낸다.
- 대사 실패가 연속되거나 manual review가 1건 이상이면 신규 결제를 OFF하고 원인을 조사한다.

## 7. 활성화 순서

1. `PAYMENTS_ENABLED=false`로 schema와 UI를 먼저 배포한다.
...
5. provider 주문·결제·취소와 내부 `payment_orders`, `payment_history`, `credit_sources`, `credit_transactions`를 대조한다.
6. test로 검증할 수 없는 수단은 승인된 운영 계정으로 최저 금액 live 결제 1건을 수행하고 즉시 원 결제수단으로 전액 취소한다.
7. 불일치 0건, 중복 지급 0건, 미완료 환불 0건을 확인한다.
8. 책임자가 심사 증빙과 외부 미확정 항목을 승인한 뒤에만 `PAYMENTS_ENABLED=true`로 전환한다.
...
- `payment_orders.refunded`와 Toss 전액 취소 결과가 일치하는지
...
1. 즉시 `PAYMENTS_ENABLED=false`로 신규 주문을 차단한다.
2. 해당 order와 source를 수동 검토 상태로 유지한다.
3. Toss 조회 결과와 내부 원장을 대조한다.
4. 원장을 직접 수정하지 말고 검토된 복구 migration 또는 전용 RPC로 처리한다.

### Toss 취소 성공 후 내부 환불 미완료

1. 동일 환불 요청과 고정 idempotency key를 유지한다.
2. Toss 결제를 조회해 전액 취소와 cancel transaction key를 확인한다.

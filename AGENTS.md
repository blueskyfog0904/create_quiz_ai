...
- 멀티 에이전트 오케스트레이션이 필요한 작업에서는 기본적으로 **서브에이전트 병렬 분석/실행**을 우선 사용하고, 단일 에이전트 fallback은 마지막 수단으로만 사용한다.
- 서브에이전트 소환/생성 시 사용자가 달리 명시하지 않는 한 **최소 `gpt-5.5` + reasoning `high`** 모델 설정을 사용한다. 역할 프롬프트가 고정 모델을 강제해 이 기준을 만족하지 못하면, 고정 역할 대신 기본 sub-agent에 `model: gpt-5.5`, `reasoning_effort: high`를 명시한다.
- 서브에이전트 **슬롯 부족**, **spawn 실패**, **ACK만 남기고 실질 시작 증거가 없는 상태**가 발생하면 그대로 포기하지 말고, 먼저 **완료/유휴/고아 서브에이전트를 정리(close/cleanup)** 해서 슬롯을 확보한 뒤 다시 spawn을 시도한다.
- spawn이 실패하면 반드시 **실패 원인(슬롯 부족, 런타임 비활성, transport 오류, 권한 문제, 컨텍스트 과포화 등)** 을 분석하고, 같은 원인으로 반복 실패하지 않도록 **정리·재시도·대체 실행 경로**를 먼저 적용한 뒤 다시 멀티 에이전트 오케스트레이션을 시도한다.
- 멀티 에이전트가 필요한 요청에서는, 서브에이전트 spawn/정리/재시도 과정을 거친 뒤에도 불가능한 경우에만 단일 에이전트로 fallback할 수 있으며, 이때는 **왜 멀티 에이전트 진행이 불가능했는지**를 사용자에게 명시한다.

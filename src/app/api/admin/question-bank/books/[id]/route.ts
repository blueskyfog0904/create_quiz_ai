...
const bookUpdateSchema = z.object({
  ...
  name: z.string().trim().min(1, '교재명은 필수입니다').optional(),
...
function notFoundResponse() {
  return NextResponse.json({ error: 'Question bank book not found' }, { status: 404 })
}
...
export async function PATCH(
  ...
  try {
    ...
    const payload: Database['public']['Tables']['question_bank_books']['Update'] = {}
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to update question bank book' }, { status: 500 })
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to deactivate question bank book' }, { status: 500 })
    }

...
## 제6조(크레딧 및 유료서비스)
크레딧(충전형 포인트)은 AI 문제 생성, 문제은행 자료 이용, 시험지 제작 및 문제마켓 자료 구매에 사용하는 서비스 내 결제 단위입니다. 크레딧 충전은 월 구독이나 자동결제가 아닌 1회 결제이며, 1회 충전금액은 100,000원 이하로 제한됩니다. 충전된 포인트의 이용기간과 환불가능기간은 결제시점으로부터 1년 이내로 제한됩니다. 충전 크레딧은 결제한 회원 계정에 귀속되어 회원 간 양도·이전하거나 현금으로 교환할 수 없습니다. 세부 환불 요건과 지원 결제수단은 취소/환불정책에서 정한 기준에 따릅니다.

회원이 결제 오류, 중복 결제, 부정 취득, 비정상 사용, 시스템 오류로 크레딧을 취득·사용한 경우 회사는 해당 크레딧을 정정, 회수, 사용 제한하거나 결제를 취소할 수 있습니다. 이미 사용한 크레딧 또는 제공이 완료된 디지털 서비스는 관련 법령과 회사의 취소/환불정책에 따라 환불이 제한될 수 있습니다.
...
## 2. 충전 관련 이용 약관
① 크레딧(포인트) 구매 이후 환불이 승인되는 경우 환불은 결제 당시 사용한 원 결제수단으로 진행됩니다.
② 구매한 크레딧(포인트)은 구매한 회원 계정에서만 사용할 수 있으며, 회원 간 양도·이전, 현금 교환 또는 명의 변경은 허용되지 않습니다.
③ 구매한 크레딧(포인트)의 사용 가능 기간은 결제일로부터 1년입니다. 기간이 경과한 크레딧은 사용이 제한될 수 있습니다.
④ 충전된 포인트의 이용기간과 환불가능기간은 결제시점으로부터 1년 이내로 제한됩니다.
⑤ 크레딧은 월 구독이나 자동결제가 아닌 1회 충전 상품이며, 1회 충전금액은 100,000원 이하로 제한됩니다.

## 3. 취소/환불 규정
① 구매 후 7일 이내이고 해당 결제로 충전된 크레딧(포인트)을 1개도 사용하지 않은 경우 결제 취소 또는 환불을 요청할 수 있습니다.
② 크레딧을 1개라도 사용해 AI 생성, 문항 저장, 문제지 생성, 자료 분석 등 유료 기능 이용이 시작 또는 완료된 경우 해당 결제 건의 취소/환불은 제한됩니다.
③ 구매한 디지털 자료, 문제지, 파일, 샘플 제외 자료를 다운로드 또는 열람한 경우 제공이 완료된 것으로 보아 환불이 제한될 수 있습니다.
④ 결제 오류, 중복 결제, 승인 실패 후 청구, 회사의 시스템 오류 또는 서비스 장애가 확인되는 경우 회사는 결제 취소, 환불 또는 크레딧 복구로 처리할 수 있습니다.
⑤ 회원이 입력한 지문, 문항 조건, 프롬프트 오류, 단순 변심, 계정 공유, 부정 사용 또는 약관 위반으로 발생한 문제는 환불이 제한될 수 있습니다.
⑥ 무상 지급 크레딧, 이벤트 포인트, 쿠폰 등 현금 결제 없이 제공된 혜택은 환불 대상에서 제외됩니다.
...
## 5. 환불 방식 및 처리 기간
크레딧 구매 이후 승인된 환불은 결제 당시 사용한 원 결제수단으로 처리하는 것을 원칙으로 하며, 임의의 다른 수단으로 전환하여 환불하지 않습니다. 카드 및 간편결제 환불은 토스페이먼츠 등 결제대행사를 통해 승인 취소 또는 환불 요청이 진행되며, 카드사·간편결제사 정책에 따라 영업일 기준 2~5일 이상 소요될 수 있습니다. 환불 승인 후 실제 입금 또는 한도 복구 시점은 결제수단 제공사의 정책에 따릅니다.

포인트 충전에는 신용·체크카드와 계약된 카카오페이, 네이버페이, 페이코, 토스페이를 사용할 수 있습니다. 계좌이체, 가상계좌 및 그 밖의 미계약 결제수단은 지원하지 않으며, 하나카드와 일부 카드사는 결제사·카드사 심사 정책에 따라 이용이 제한될 수 있습니다.
...
## 8. 충전 경로와 사용 경로
- 충전 경로: 로그인 → 크레딧 충전 상품(/pricing) → 충전 상품 선택 → 결제(/checkout) → 토스페이먼츠 결제 승인 → 마이페이지 크레딧(/mypage/credits)에서 충전 내역 확인
- 사용 경로: 충전된 크레딧은 AI 문제 생성, 문제은행 자료 이용·가져오기, 시험지 제작 및 영어·국어 문제마켓 자료 구매에 사용됩니다. 사용·차감 내역은 마이페이지 크레딧(/mypage/credits), 결제 상태는 결제 내역(/mypage/payments)에서 확인할 수 있습니다.
- 문제마켓 구매 취소 시에는 현금으로 별도 지급하지 않고, 원래 차감된 유효 크레딧 source로 복구하는 별도 정책이 적용됩니다.

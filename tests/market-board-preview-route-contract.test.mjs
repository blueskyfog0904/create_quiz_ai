...
test('board preview client presents a compact board-style list with purchase controls', () => {
  ...
  assert.match(source, /py-2/, 'board rows should use compact vertical padding after metadata removal')
  assert.doesNotMatch(source, /formatExamMeta/, 'board rows should not render exam metadata under titles')
  assert.doesNotMatch(source, /Sparkles/, 'board rows should not render sample badges under titles')

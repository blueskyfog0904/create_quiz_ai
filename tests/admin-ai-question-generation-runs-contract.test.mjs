...
test('admin AI generation run pages show list, detail, filters, and JSON download controls', () => {
  ...
  assert.match(detailPageSource, /QuestionPreview/)
  assert.match(detailPageSource, /QuestionGenerationAttemptLogViewer/)
  assert.match(detailPageSource, /safeAttemptLogs/)
  assert.match(detailPageSource, /getPreviewQuestion/)
  assert.match(detailPageSource, /getReviewResult/)
  assert.match(detailPageSource, /최종 통과 문제/)
  assert.match(detailPageSource, /마지막 생성 문제/)
  assert.match(detailPageSource, /마스킹/)
  assert.match(detailPageSource, /축약/)
  assert.match(detailPageSource, /만료/)

...
const filledTemplate = readSource('src/lib/question-bank/filled-template.ts')
...
test('bulk upload route remains parse-only and resolves active year/book metadata', () => {
  ...
  assert.match(bulkUploadRoute, /지문앞텍스트/)
  assert.match(bulkUploadRoute, /지문뒤텍스트/)
  assert.match(bulkUploadRoute, /문제앞텍스트/)
  assert.match(bulkUploadRoute, /문제뒤텍스트/)
...
test('template route documents year, book name, and problem-type-name columns with active reference sheets', () => {
  assert.match(templateRoute, /QUESTION_UPLOAD_TEMPLATE_HEADERS/)
  assert.match(filledTemplate, /'year'/)
  assert.match(filledTemplate, /'교재명'/)
  assert.match(filledTemplate, /bankProblemTypeId/)
  assert.match(filledTemplate, /'지문앞텍스트'/)
  assert.match(filledTemplate, /'지문뒤텍스트'/)
  assert.doesNotMatch(filledTemplate, /'문제앞텍스트'/)
  assert.doesNotMatch(filledTemplate, /'문제뒤텍스트'/)

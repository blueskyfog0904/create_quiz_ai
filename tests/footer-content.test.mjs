...
test('default refund policy covers this service-specific credit and digital content rules', () => {
  ...
  assert.match(refundPolicy, /충전 관련 이용 약관/)
  assert.match(refundPolicy, /취소\/환불 규정/)
  ...
  assert.match(refundPolicy, /1개도 사용하지/)
  assert.match(refundPolicy, /사용 가능 기간|사용 기한/)
  ...
  assert.doesNotMatch(refundPolicy, /결제일로부터 1년 이내에 미사용 크레딧.*환불/)
